...
void skill4(vector<Card*> &field1,vector<Card*> &field2, Card* card) // Muccuk
{
    ...
    if(asasvarmi == true)
    {
        ...
        cout << "Muccuk is using his skill! Muccuk stuns and gives " << card->getAtk()/2 << " damage to " << field1[asasindex]->name << ". " <<  field1[asasindex]->name << "'s remaining hp: " <<  field1[asasindex]->getHp() << "\n" ;
    ...
    else
    {
        ...
        cout << "Muccuk is using his skill! Muccuk stuns and gives " << card->getAtk()/2 << " damage to " << field1[0]->name << ". " <<  field1[0]->name << "'s remaining hp: " <<  field1[0]->getHp() << "\n" ;
...
void skill7(vector<Card*> &field1,vector<Card*> &field2, Card* card) // Umai
{
    ...
    if(field1.size() > 2)
    {
        ...
        cout << "Umai is using his skill! Umai's attack gives " << atk << " area damage to " << field1[0]->name << ", " << field1[1]->name << " and " <<  field1[2]->name << ".\n" ;
    }
    else if(field1.size() == 2)
    {
        ...
        field1[0]->setHp(field1[0]->getHp()- atk) ;
        field1[1]->setHp(field1[1]->getHp()- atk) ;
        cout << "Umai is using his skill! Umai's attack gives " << atk << " area damage to " << field1[0]->name << " and " << field1[1]->name << ".\n" ;
    }
    else if(field1.size() == 1)
    {
        atk = card->getAtk() * 6/5;
        field1[0]->setHp(field1[0]->getHp()- atk) ;
         cout << "Umai is using his skill! Umai's attack gives " << atk << " single target damage to " << field1[0]->name << ".\n" ;
    }
...
void skill10(vector<Card*> &field1,vector<Card*> &field2, Card* card) // Atreus
{
    ...
    card->setAtk(card->getAtk()+5) ;
    cout << "Atreus is using his skill! Atreus attacked " << field1[encokindex]->name << " for " << card->getAtk() +5 << " damage. " << field1[encokindex]->name << "'s remaining hp: " << field1[encokindex]->getHp() << ". Atreus increased his attack damage by 5.\n" ;
